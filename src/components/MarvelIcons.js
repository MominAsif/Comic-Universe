...
const DCIcons = (props) => {
  ...
  const imageResults = async function (image) {
    try {
      const response = await API.get(
        `http://comicvine.gamespot.com/api/issues/?api_key={api key here}&filter=image:${image}&field_list=image`,
        {
          params: { format: "json", api_key: "{api key here}" }
        }
      );
  ...
  const searchApi = async function (name) {
    try {
      const response = await API.get(
        `https://comicvine.gamespot.com/api/issues/?api_key{api key here}=&filter=name:${name}&format=json`,
        {
          params: { 
          format: "json", 
